...
const Upload_api = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (files.length === 0) {
      return toastify("No hay imagenes", false);
    }
    const uploadedUrls = await Promise.all(
      files.map(async (file, index) => {
        ...
        try {
          const response = await urlAxios.post(`converter/${id}`, formDataImg, {
            headers: {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-8 py-10">
          ...
          {files.length > 0 &&
            ...
            files.map((file, index) => (
              <div className="rounded-md bg-[#F5F7FB] py-4 px-8" key={index}>
